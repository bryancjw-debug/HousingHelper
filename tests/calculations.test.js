...
const belowValuation = input({ propertyType: "privateCondo", loanType: "bank", purchasePrice: 1200000, marketValue: 1000000 });
...
const directHdb = input({ propertyType: "hdbBto", purchasePrice: 500000, marketValue: 400000 });
assert.equal(hh.effectiveMarketValue(directHdb), 500000, "direct HDB purchase uses its purchase price as the modelled value");
assert.equal(hh.calculate(directHdb).cov, 0, "BTO and SBF purchases do not model cash over valuation");
...
assert.equal(defaultResult.cpfUsed, 69200, "default CPF use starts without an automatic OA reserve");
assert.equal(defaultResult.reno, 0, "renovation is excluded until the user opts in");
assert.equal(defaultResult.cashRequired, 0, "default purchase is fully covered without optional renovation");
assert.equal(defaultResult.cashSurplus, 100000, "default profile keeps unused cash visible");
const renovationIncluded = hh.calculate(input({ includeRenovation: true }));
assert.equal(renovationIncluded.reno, 25000, "light renovation and move-in costs are included after opt-in");
assert.equal(renovationIncluded.cashRequired, 25000, "opted-in renovation remains a cash outlay");
assert.equal(hh.affordablePrice(input()), 706000, "default estimated maximum price remains reproducible without optional reserves");
assert.equal(Math.round(hh.calculate({ ...input(), purchasePrice: 706000, marketValue: 706000, useMaxLoan: true }).selected.loan), 506103, "maximum affordable price translates to its corresponding maximum usable loan");
assert.equal(hh.affordablePrice(input({ monthlyIncome: 2000 })), 338000, "maximum price scales down with lower servicing income");
assert.equal(hh.affordablePrice(input({ monthlyIncome: 4000 })), 461000, "maximum price scales up as income rises");
assert.equal(hh.affordablePrice(input({ monthlyIncome: 6000 })), 583000, "maximum price continues scaling with servicing income");
assert.equal(hh.affordablePrice(input({ monthlyIncome: 8000 })), 706000, "maximum price reflects both income capacity and entered funds");
const lowCashTarget = hh.calculate(input({ propertyType: "hdbBto", purchasePrice: 500000, marketValue: 400000, cashAvailable: 0, cpfAvailable: 80000 }));
assert.equal(lowCashTarget.selected.loan, 375000, "desired BTO target retains its maximum LTV loan when cash is short");
assert.equal(lowCashTarget.cashShortfall, 58100, "desired BTO target reports the exact cash funding gap");
assert.ok(hh.affordablePrice(input({ cashAvailable: 0, cpfAvailable: 80000 })) > 0, "low cash no longer collapses the current-funds property range to zero by default");
